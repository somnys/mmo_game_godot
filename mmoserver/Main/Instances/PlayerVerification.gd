...
func Verify(player_id, token: String):
	...
	while int(Time.get_unix_time_from_system()) - int(token.right(-64)) <=30:
		if main_interface.expected_tokens.has(token):
			...
			yield_timer.start()
			await yield_timer.timeout
	main_interface.GetTokenVerificationResult(player_id, token_verification)
	...
	if awaiting_verification == {}:
		...
		for key in awaiting_verification.keys():
			...
			if current_time - start_time >= 10:
				...
				var connected_peers = Array(get_tree().get_multiplayer().get_peers()) #TUTAJ!!!
				if connected_peers.has(key):
					main_interface.GetTokenVerificationResult(key,false)
